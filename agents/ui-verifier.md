...
- I1: `∀ test_data := Bash(psql ∨ sqlite3 ∨ mysql) ∨ Read(fixture). ¬invent. ¬hardcode.`
- I2: `∀ FK_value : ∃ row ∈ referenced_table via Bash(psql ∨ sqlite3 ∨ mysql). ¬confirmed → ¬use.`
...
- I9: `∀ interaction_requiring_input : input := Bash(psql ∨ sqlite3 ∨ mysql) ∨ Read(fixture) ∨ (verdict = FAIL("failure to get data: " + interaction) ∧ record(testability_gaps[{affordance, reason}])).`
...
`describe_table(target)` → columns + FK relationships. `select` → fetch valid FK rows. Enforce I1 + I2.
